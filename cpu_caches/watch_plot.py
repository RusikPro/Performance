...
def read_and_plot(filename, ax):
    ...
    Assumes the file has the following structure:
      ...
      Row 1: "Container", time1, time2, ..., timeN
      Row 2: "LocalCounter", time1, time2, ..., timeN
    """
    try:
    ...
    try:
        ...
        container_times = list(map(float, df.iloc[1, 1:].tolist()))
        localcounter_times = list(map(float, df.iloc[2, 1:].tolist()))
    except Exception as e:
        print(f"Error processing CSV data: {e}")
        return

    ax.clear()
    ax.plot(thread_counts, container_times, marker='o', label='Container')
    ax.plot(thread_counts, localcounter_times, marker='s', label='LocalCounter')
...
def main(filename):
    ...
    # Use a blocking show() call so we don't have to poll continuously.
    plt.show(block=True)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python watch_plot.py <benchmark_file.csv>")
        sys.exit(1)
    main(sys.argv[1])
